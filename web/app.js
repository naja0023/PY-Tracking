...
app.post('/loginmoblie', function (req, res) {
  ...
  const sql =
    'SELECT * FROM driver LEFT JOIN car_match on driver.driver_id = car_match.driver_id WHERE driver.username =? AND DATE(car_match.date) = DATE(CURRENT_TIMESTAMP()) AND role=2;'
...
app.post('/review', (req, res) => {
  ...
  con.query(
    sql,
    [driver_id, user_email, user_name, point, report],
    (err, result) => {
      if (err) {
        console.log(err)
        res.status(503).send('Server error')
...
client.on('connect', function () {
  con.query(
    'SELECT * FROM car_match where DATE(date)=DATE(CURRENT_TIMESTAMP())',
    function (err, result) {
      if (result.length > 0) {
        // console.log(result[i].driver_id)
        for (var i = 0; i < result.length; i++) {
          let driver_id = result[i].driver_id
          let car_id = result[i].car_id
          client.subscribe(
            'car' + driver_id + car_id,
            (err) => {
              if (!err) {
                client.publish('car' + driver_id + car_id, 'Hello Car1')
              }
            },
            // console.log('Car' + result[i].driver_id + result[i].car_id),
          )
        }
      }
    },
